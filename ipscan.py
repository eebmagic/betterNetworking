...
if len(sys.argv[:]) > 1 and sys.argv[1] and sys.argv[1] not in commandOptions:
    print(f"no such option: '{sys.argv[1]}'\n\trun ipscan --help to see all options")
    # print(f"len of sys.argv: {len(sys.argv[:])}")		#Debugging
    # print(f"sys.argv[1]: {sys.argv[1]}")				#Debugging
    quit()
...
if '--add' in sys.argv[:]:
    ...
    back = user_PATH[::-1]
    currentUser = back[:back.index('/')][::-1]
    user_mainUserAccount = input(f"What should be the main account used for ssh/scp? (Default: {currentUser}): ").strip()
    if user_mainUserAccount == '':
    	user_mainUserAccount = currentUser

    user_defaultDirectory = input("What directory to scp to?(Default: ~/Downloads/): ").strip()
    if user_defaultDirectory == '':
    	user_defaultDirectory = "~/Downloads/"


    currentData[user_newName] = {'mac':user_newMAC, 'ip':"None", 'user':user_mainUserAccount, 'folder':user_defaultDirectory}
...
for row in rows:
	if getMAC(row):
		...
		if witharp:
			print(f"{getMAC(row) + ' '*(17 - len(getMAC(row)))}  |  {getIP(row)}")  #Print all collected arp data

#Compare record with arp data
for name, data in oldData.items():
	#Check in arp data
	if data['mac'] in arpMac_ips:
		currentData[name]['ip'] = arpMac_ips[data['mac']]	#Update ip in new Data
		currentData[name]['connection'] = True				#Update connection status
...
for name, data in sorted(currentData.items()):
	...
	print(f"{device_name}| {device_mac} |  {device_ip}  | {device_connect}")
